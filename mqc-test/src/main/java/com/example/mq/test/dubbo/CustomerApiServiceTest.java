...
public class CustomerApiServiceTest {
	...
	private void testQueryCustomer(){
		...
		try {
			CustomerClient customerService = CustomerApiServiceTest.getDubboService(CustomerClient.class, null);
			mqcResponse = customerService.queryCustomerList(request);
		...
		if(null == mqcResponse){
			...
			System.out.println("------ query result:" + JSONObject.toJSONString(mqcResponse));
		}
	}
