package com.example.mq.infra.zk;
...
public class CuratorClientManager {
			...
			synchronized (CuratorFramework.class){
				if(Objects.isNull(ZK_CLIENT)){
					ZK_CLIENT = CuratorFrameworkFactory.builder()
							.connectString(zkConnConfig.getZkAddress())
							.sessionTimeoutMs(zkConnConfig.getSessionTimeoutMs())
							.connectionTimeoutMs(zkConnConfig.getConnectionTimeoutMs())
							.retryPolicy(new ExponentialBackoffRetry(zkConnConfig.getSleepTime(), zkConnConfig.getMaxRetries()))
							.namespace(zkConnConfig.getNamespace())
							.build();
					ZK_CLIENT.start();
				}
			}
	...
	public Stat update(String zkPath, byte[] bytes) throws Exception{
		...
		if(!this.isNodeExist(zkPath)){
			throw new BusinessException(String.format("zk节点不存在，zkPath:%s", zkPath));
		}
