...
public class StockIndicatorManagerImpl implements StockIndicatorManager {
        ...
        List<AnalyseIndicatorDTO> analyseIndicatorDTOList = indicatorDTOList.stream()
                ...
                .filter(indicatorDTO -> {
                    if (indicatorDTO.getOperating_income_yoy() != null && indicatorDTO.getOperating_income_yoy() <= 1.5) {
                        ...
                        // 当季营收下滑，则要求当季净利润同比增加
                        if (indicatorDTO.getCur_q_net_profit_atsopc_yoy() != null && indicatorDTO.getCur_q_net_profit_atsopc_yoy() >= 0) {
                            return true;
                        }

                        // 当季毛利率、净利率同比好转也可
                        if (indicatorDTO.getCur_q_gross_margin_rate_change() != null && indicatorDTO.getCur_q_gross_margin_rate_change() >= 0.1
                                && indicatorDTO.getCur_q_net_selling_rate_change() != null && indicatorDTO.getCur_q_net_selling_rate_change() >= 0.1) {
                            return true;
                        }
    ...
    private void formatIndNorthHoldShareDTO(DongChaiIndustryHoldShareDTO holdShareDTO) {
        ...
        if (holdShareDTO.getIndustryRatio() != null) {
            double industryRatio = NumberUtil.format(holdShareDTO.getIndustryRatio(), 3);
            holdShareDTO.setIndustryRatio(industryRatio);
        }
        if (holdShareDTO.getInd_ratio_chg_30() != null) {
            double ind_ratio_chg_30 = NumberUtil.format(holdShareDTO.getInd_ratio_chg_30(), 3);
            holdShareDTO.setInd_ratio_chg_30(ind_ratio_chg_30);
        }
        if (holdShareDTO.getInd_ratio_chg_90() != null) {
            double ind_ratio_chg_90 = NumberUtil.format(holdShareDTO.getInd_ratio_chg_90(), 3);
            holdShareDTO.setInd_ratio_chg_90(ind_ratio_chg_90);
        }
        if (holdShareDTO.getInd_ratio_chg_360() != null) {
            double ind_ratio_chg_360 = NumberUtil.format(holdShareDTO.getInd_ratio_chg_360(), 3);
