package com.example.mq.data.mySqlConf;
...
public class MySqlConfigUtil {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(MySqlConfigUtil.class);
    ...
    static {
        ...
        try {
            InputStream in = MySqlConfigUtil.class.getClassLoader().getResourceAsStream("mySql.properties");
            properties.load(in);
        } catch (IOException e) {
        ...
        // 连接mysql数据库
        MysqlConf mysqlConf = new MysqlConf(properties);
        Connection mysqlConn = null;
        String url = String.format(CONN_URL_FORMAT, mysqlConf.getHost(), String.valueOf(mysqlConf.getPort()),
                mysqlConf.getDb());
        try {
            mysqlConn = DriverManager.getConnection(url, mysqlConf.getUser(), mysqlConf.getPassword());
